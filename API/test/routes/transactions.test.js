...
test('Teste #14 - Listar todas as transaçoes de utilizador', () => {
...
test('Teste #15 - Listar uma transaçoes', () => {
  return app.db('transactions').insert(
...
//teste vai para gameusers
test('Teste #17 - Tentar uma transaçao de compra sem saldo suficiente', async () => {
  const res = await app.services.gameUsers.save({ user_id: userC.id, game_id: testGameA.id, cash: 0});
  testGameAUserC = { ...res3[0] };

  return request(app).post(MAIN_ROUTE)
  .set('authorization', `bearer ${userC.token}`)
  .send({
    games_users_id: testGameAUserC.id, crypto_id: testCrypto.id, date: new Date(),  type: 'B', amount: 5, crypto_value: 100 
  })
  .then((res) => {
...
describe('Validação de criar uma transação', () => {
  ...
  test('Test 9.1 - Sem Game_User_ID', () => testTemplate({ games_users_id: null }, 'O GAME_USER é um atributo obrigatório'));
  test('Test 9.2 - Sem Crypto_ID', () => testTemplate({ crypto_id: null }, 'O Crypto_ID é um atributo obrigatório'));
  test('Test 9.3 - Sem Data', () => testTemplate({ date: null }, 'A DATA é um atributo obrigatório'));
  test('Test 9.4 - Sem Tipo', () => testTemplate({ type: null }, 'O TIPO é um atributo obrigatório'));
  test('Test 9.4 - Com Tipo errado', () => testTemplate({ type: 'P' }, 'O TIPO tem um valor inválido'));
  test('Test 9.5 - Sem Amount', () => testTemplate({ amount: null }, 'O AMOUNT é um atributo obrigatório'));
  test('Test 9.6 - Sem o valor da crypto', () => testTemplate({ crypto_value: null }, 'O VALOR DA CRYPTO é um atributo obrigatório'));
});
